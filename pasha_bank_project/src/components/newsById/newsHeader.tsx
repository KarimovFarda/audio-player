...
export const NewsHeader = () => {
    ...
    return (
        ...
        {id <= 8 ? headData && headData.slice(0.8).map((item: object, index: number) => {
          ...
          }) : id < (29 + state.news.length) ? headData && newsdata && state && state.news.map((item: object, index: number) => {
          ...
          }) : info && headData && newsdata && state && info.articles.map((item: object, index: number) => {
