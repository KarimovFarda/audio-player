...
export const NewsContent = () => {
    ...
    return (
        <div className='article-content' style={{ opacity: "1" }}>
          {id <= 8 ? headData && headData.slice(0.8).map((item: object, index: number) => {
          ...
          }) : id < 29 ? headData && newsdata && newsdata.map((item: object, index: number) => {
          ...
          }) : id < (29 + state.news.length) ? state && headData && newsdata && state.news.map((item: any, index: number) => {
          ...
          }) : info && state && headData && newsdata && info.articles.map((item: object, index: number) => {
            if (Number(id) === Number(index + 30 + state.news.length)) {
          ...
          }) : id < 31 ? state && headData && newsdata && state.news.map((item: any, index: number) => {
            if (Number(id) === Number(index + 29)) {
          ...
          }) : info && state && headData && newsdata && info.articles.map((item: any, index: number) => {
