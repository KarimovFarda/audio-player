...
class Map extends Component<any> {
  ...
  componentDidUpdate(prevProps: any) {
    const { query }: any = this.props;
    if (query !== prevProps.query) {
      this.prepareData();
    }
  }
  prepareData = () => {
    const { colors, data, query }: any = this.props;
    const map_data = data.filter((f: any) => f[query] > 0);
    const counts = map_data.map((e: any) => e[query]);
  ...
  render() {
    ...
    return (
      ...
      >
        {map_data.map((country: any, index) => {
