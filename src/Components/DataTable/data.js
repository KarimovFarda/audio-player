...
export default function DataTable({records,headers}) {
    ...
    <a class="nav-link text-danger font-weight-bold" href="/">LOG OUT</a>
        ...
        <div style={{padding:0.5 + "rem"}} className="card-body">
            <table className="table" style={{overflow:"hidden"}}>
                ...
                <tr className={'bg-light'} style={{color:'darkorange'}}>
                      <th  style={{fontSize : 0.9 + "rem",color:'darkorange'}} scope="row">{index + 1}</th>
                      <td  style={{fontSize : 0.9 + "rem",color:'darkorange'}} className="font-weight-bold">{(item[1].split(",")[0])}</td>                      
                          <td style={{fontSize : 0.9 + "rem",color:'darkorange'}} className="font-weight-bold"> {(item[1].split(",")[1])}</td>
                          <td  style={{fontSize : 0.9 + "rem",color:'darkorange'}}className="font-weight-bold ">{(item[1].split(",")[2])}</td>
                          <td style={{fontSize : 0.9 + "rem",color:'darkorange'}} className="font-weight-bold">{(item[1].split(",")[3])}</td>
                          
                          <td style={{fontSize : 0.9 + "rem",color:'darkorange'}} className="font-weight-bold ">{(item[1].split(",")[5])}</td>
                          <td style={{fontSize : 0.9 + "rem",color:'darkorange'}} className="font-weight-bold ">{(item[1].split(",")[4])}</td>

                          <td style={{fontSize : 0.9 + "rem"}}><Link to={'/data/'} className="btn btn-warning font-weight-bold text-dark"> Details</Link></td>
                          <td style={{fontSize : 0.9 + "rem"}}><Link to={'/edit/' + index} className="btn btn-warning font-weight-bold text-dark"> Edit</Link></td>
                    </tr>
                     ))}
                   {computedRecords.map((record,index) =>(
                      
                  <tr style={{cursor:'pointer'}} className={'bg-light font-weight-bold'} key={record.login.uuid}>
                  <th style={{fontSize : 0.9 + "rem"}} scope="row">{index + (localStorage.length + 1) + (currentPage - 1) * itemsPerPage}</th>
                  <td style={{fontSize : 0.9 + "rem"}}>{record.name.title == "Mademoiselle" || record.name.title == "Madame" ? "Ms" : record.name.title  }</td>
                  <td style={{fontSize : 0.9 + "rem"}}>{record.gender}</td>
                  <td style={{fontSize : 0.9 + "rem"}}>{record.name.first} {record.name.last}</td>
                  <td style={{fontSize : 0.9 + "rem"}}>{record.email}</td>
                  <td style={{fontSize : 0.9 + "rem"}}>{record.location.country}</td>
                  <td style={{fontSize : 0.9 + "rem"}}>{record.login.username}</td>
                     <td><Link to={'/data/' + (index + localStorage.length)} className="btn btn-warning font-weight-bold text-dark"> Details</Link></td>
                     <td><Link to={'/edit/' + (index + localStorage.length)} className="btn btn-warning font-weight-bold text-dark"> Edit</Link></td>
              </tr>                         
                   ))      
                   }                     
                </tbody>
